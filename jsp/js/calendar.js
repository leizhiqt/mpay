...
function displayCalendar(obj, strFormat) {
	var date = new Date();
...
Calendar.prototype.bindData = function() {
	var calendar = this;
	var dateArray = this.getMonthViewArray(this.date.getYear(), this.date
			.getMonth());
	...
	for ( var i = 0; i < tds.length; i++) {
		...
		if (dateArray[i] != "&nbsp;") {
			...
			if (new Date().format(calendar.dateFormatStyle) == new Date(
...
Calendar.prototype.show = function(dateObj, popControl) {
	...
	this.date = (dateObj.value.length > 0) ? new Date(dateObj.value
			.toDate(this.dateFormatStyle)) : new Date();//2006-12-03 寒羽枫添加 → 若为空则显示当前月份
	this.year = this.date.getFullYear();
	this.month = this.date.getMonth();
	this.changeSelect();
	this.bindData();
...
document
		.write('background-color: #FFFFFF;border: 1px solid #CCCCCC;width:175px;font-size:12px;"></div>');
if (document.all) {
	document
