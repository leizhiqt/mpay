...
<table class="modify">
<caption><fmt:message key="Add"/> <fmt:message key="WineJar"/></caption>
...
<tr>
<td class="tl"><fmt:message key="Suffix"/></td>
<td><input type="text" name="suffixJarNumber"  size="1" maxlength="1"/></td>
</tr>
...
<td>
<input type="text" name="wineJar.price" value="${wineJar.price}" size="10" maxlength="10"/>

<select name="wineJar.orgCurrencyId">
	<c:forEach var="items" items="${currencys}" varStatus="s">
		<option value="${items.key}"

		<c:if test="${items.key==2}">
			selected="selected"
		</c:if>
			>
		${items.value}
...
<td>
<input type="text" name="wineJar.createDate" value="<fmt:formatDate value="${wineJar.createDate }" type="both"/>" size="19" maxlength="19" onclick="displayCalendar(this,'yyyy-MM-dd hh:mm:ss');"/>
<font class="fm">*</font>
</td>
</tr>
